...
cdef class BaseQRMUMPSSolver_INT32_FLOAT64:
    ...
    def __dealloc__(self):
        ...
        dqrm_spmat_destroy_c(&self.params)
    ...
    def analyze(self, ordering='auto'):
        ...
        # Set ordering option
        dqrm_pseti_c(&self.params, "qrm_ordering", orderings[ordering])

        t1 = time.clock()
        dqrm_analyse_c(&self.params, self.transp)
        t2 = time.clock()
    ...
    def factorize(self, ordering=None, pivot_tol=0.01):
        ...
        t1 = time.clock()
        dqrm_factorize_c(&self.params, self.transp)
        t2 = time.clock()
    ...
    def solve(self, rhs, compute_residuals=False):
        ...
        if rhs.dtype != np.float64:
            raise TypeError("Type mismatch! Right hand side must be of type FLOAT64")
        ...
        if rhs.ndim==1:
            nrhs = 1 
            x = np.zeros(self.ncol, dtype=np.float64) 
        elif rhs.ndim==2:
            nrhs = rhs_shape[1]
            x = np.zeros([self.ncol, nrhs], order='F', dtype=np.float64)
        else:
        ...
        if self.transp=='n':
            dqrm_apply_c(&self.params, 't', <double *> cnp.PyArray_DATA(b), nrhs);
            dqrm_solve_c(&self.params, 'n', <double *> cnp.PyArray_DATA(b), <double *> cnp.PyArray_DATA(x), nrhs);
        elif self.transp=='t':
            dqrm_solve_c(&self.params, 't', <double *> cnp.PyArray_DATA(b), <double *> cnp.PyArray_DATA(x), nrhs);
            dqrm_apply_c(&self.params, 'n', <double *> cnp.PyArray_DATA(x), nrhs);
        ...
        if compute_residuals:
            ...
            dqrm_residual_norm_c(&self.params,
    ...
    def least_squares(self, rhs):
        ...
        dqrm_least_squares_c(&self.params, <double *> cnp.PyArray_DATA(b), <double *> cnp.PyArray_DATA(x), nrhs)
        return x
    ...
    def minimum_norm(self, rhs):
        ...
        dqrm_min_norm_c(&self.params, <double *> cnp.PyArray_DATA(b), <double *> cnp.PyArray_DATA(x), nrhs)
        return x
...
#TODO: test if factorise has been called
        ...
        for i in xrange(0,niter):
            dqrm_matmul_c(&self.params, 'n', <double> -1,
                                                      <double *> cnp.PyArray_DATA(new_x), 
                                                      <double> 1,
                                                      <double *> cnp.PyArray_DATA(residuals),
                                                      nrhs)

            if self.transp=='n':
                dqrm_apply_c(&self.params, 't', <double *> cnp.PyArray_DATA(residuals), nrhs);
                dqrm_solve_c(&self.params, 'n', <double *> cnp.PyArray_DATA(residuals), <double *> cnp.PyArray_DATA(new_x), nrhs);
            elif self.transp=='t':
                dqrm_solve_c(&self.params, 't', <double *> cnp.PyArray_DATA(residuals), <double *> cnp.PyArray_DATA(new_x), nrhs);
                dqrm_apply_c(&self.params, 'n', <double *> cnp.PyArray_DATA(new_x), nrhs);
