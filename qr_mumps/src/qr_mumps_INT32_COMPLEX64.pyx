...
cdef class BaseQRMUMPSSolver_INT32_COMPLEX64:
    ...
    def __dealloc__(self):
        ...
        cqrm_spmat_destroy_c(&self.params)
    ...
    def analyze(self, ordering='auto'):
        ...
        # Set ordering option
        cqrm_pseti_c(&self.params, "qrm_ordering", orderings[ordering])

        t1 = time.clock()
        cqrm_analyse_c(&self.params, self.transp)
        t2 = time.clock()
    ...
    def factorize(self, ordering=None, pivot_tol=0.01):
        ...
        t1 = time.clock()
        cqrm_factorize_c(&self.params, self.transp)
        t2 = time.clock()
    ...
    def solve(self, rhs, compute_residuals=False):
        ...
        """
        if rhs.dtype != np.complex64:
            raise TypeError("Type mismatch! Right hand side must be of type COMPLEX64")
        ...
        if rhs.ndim==1:
            nrhs = 1 
            x = np.zeros(self.ncol, dtype=np.complex64) 
        elif rhs.ndim==2:
            nrhs = rhs_shape[1]
            x = np.zeros([self.ncol, nrhs], order='F', dtype=np.complex64)
        else:
        ...
        if self.transp=='n':
            cqrm_apply_c(&self.params, 't', <float complex *> cnp.PyArray_DATA(b), nrhs);
            cqrm_solve_c(&self.params, 'n', <float complex *> cnp.PyArray_DATA(b), <float complex *> cnp.PyArray_DATA(x), nrhs);
        elif self.transp=='t':
            cqrm_solve_c(&self.params, 't', <float complex *> cnp.PyArray_DATA(b), <float complex *> cnp.PyArray_DATA(x), nrhs);
            cqrm_apply_c(&self.params, 'n', <float complex *> cnp.PyArray_DATA(x), nrhs);
        ...
        if compute_residuals:
            ...
            cqrm_residual_norm_c(&self.params,
                                                             <float complex *> cnp.PyArray_DATA(residuals),
                                                             <float complex *> cnp.PyArray_DATA(x),
                                                             nrhs,
    ...
    def least_squares(self, rhs):
        ...
        cqrm_least_squares_c(&self.params, <float complex *> cnp.PyArray_DATA(b), <float complex *> cnp.PyArray_DATA(x), nrhs)
        return x
    ...
    def minimum_norm(self, rhs):
        ...
        cqrm_min_norm_c(&self.params, <float complex *> cnp.PyArray_DATA(b), <float complex *> cnp.PyArray_DATA(x), nrhs)
        return x
...
#TODO: test if factorise has been called
        ...
        for i in xrange(0,niter):
            cqrm_matmul_c(&self.params, 'n', <float> -1,
                                                      <float complex *> cnp.PyArray_DATA(new_x), 
                                                      <float> 1,
                                                      <float complex *> cnp.PyArray_DATA(residuals),
                                                      nrhs)

            if self.transp=='n':
                cqrm_apply_c(&self.params, 't', <float complex *> cnp.PyArray_DATA(residuals), nrhs);
                cqrm_solve_c(&self.params, 'n', <float complex *> cnp.PyArray_DATA(residuals), <float complex *> cnp.PyArray_DATA(new_x), nrhs);
            elif self.transp=='t':
                cqrm_solve_c(&self.params, 't', <float complex *> cnp.PyArray_DATA(residuals), <float complex *> cnp.PyArray_DATA(new_x), nrhs);
                cqrm_apply_c(&self.params, 'n', <float complex *> cnp.PyArray_DATA(new_x), nrhs);
